...
class BoostingTree:
    ...
    def split_arr(self, X_data):
        ...
        print(X_data[:, 0])   # x 是10*1 的二维list
        print(np.where(X_data[:, 0] <= 3.5))
 
        # 切成两部分
        for split in self.candidate_splits:
            left_index = np.where(X_data[:, 0] <= split)[0]  # 把x样本第0列取出变成一维list，同时把小于split的下标索引值保留; 形成np数据：1 * SPLIT 维度，取出第1行下标索引；
    ...
    def calculate_error(self, split, y_result):
        ...
        result_square = np.apply_along_axis(lambda x: x ** 2, 0, result).sum()  # 0列方向压扁统计,使用function为lambda x : x**2
