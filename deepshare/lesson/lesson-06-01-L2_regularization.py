...
方差： variance 描述数据的扰动造成的影响，  测试集和训练集表现有差异(过拟合)，测试集上表现非常差； 正则化的目的：降方差，减少过拟合


稀疏解： 加上L1可能产生稀疏解，因为参数解最小可能发生在坐标轴上

weight decay ： 权重衰减   L2 regularization

Obj = Cost + Regulariztion term

为什么使用regulariztion会实现：权值衰减的功能？


这个实验目的是：使用正则和不使用正则的情况下，是否过拟合的对比？
...
def gen_data(num_data=10, x_range=(-1, 1)):
    ...
    print("train_x size： {}".format(train_x))

    print("train_y size： {}".format(train_y))
